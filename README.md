...
캐릭터 선택에서는 같은 씬의 카메라가 캐릭터 정면으로 붙는다.
...
이름·칭호·대사·뒷이야기는 [js/characters.js](js/characters.js) 한 곳에 모여 있다.
항목을 추가하면 명단·스탯·3D 모델이 함께 늘어난다.

### 폰트

영문/숫자는 **Press Start 2P**(Google Fonts), 한글은 **Galmuri**(OFL, jsDelivr).
한글 글리프가 없는 영문 픽셀 폰트를 앞에 두고 갈무리로 폴백시켜 두 서체를 섞는다.
